...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] TransformNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is TransformNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (TransformNetworkObject.Length > 0 && TransformNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(TransformNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<TransformBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateTransform instead, its shorter and easier to type out ;)")]
		public TransformBehavior InstantiateTransformNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TransformNetworkObject[index]);
			var netBehavior = go.GetComponent<TransformBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<TransformBehavior>().networkObject = (TransformNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Transform
		/// </summary>
		/// <returns>
		/// A local instance of TransformBehavior
		/// </returns>
		/// <param name="index">The index of the Transform prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public TransformBehavior InstantiateTransform(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(TransformNetworkObject[index]);
			var netBehavior = go.GetComponent<TransformBehavior>();
